<div>
    <div class="bg-indigo-600 text-white items-center mb-2 px-4 py-2 border border-transparent text-sm md:text-base leading-6 rounded-md font-medium w-full" wire:loading wire:target="inputTempFiles">
    ...
    @error('inputTempFiles.*')
        <div class="w-full text-sm md:text-base bg-red-200 border border-red-700 text-red-800 rounded-sm py-3 px-4 mb-2">
    ...
    <div>
        @if (session()->has('message'))
            <div class="text-sm md:text-base @if(session()->has('type_message')) @if(session('type_message') == 'error') bg-red-200 border border-red-700 text-red-800 @endif @else bg-green-200 border-green-700 text-green-800 @endif w-full border rounded-sm py-3 px-4 mb-2">
    ...
    <div class="relative flex flex-col flex-grow mb-3">
        <div class="flex flex-col w-full bg-white border-2 border-gray-300 border-solid rounded-md hover:shadow-outline-gray cursor-pointer py-3 text-center">
            ...
            <p class="text-gray-700 text-sm md:text-base">Suelta tus archivos aqui o haz clic en esta area.</p>
            <a href="javascript:void(0)" class="mt-3 mx-auto py-1 md:py-2 px-4 text-sm md:text-base text-white text-center font-medium border border-transparent rounded-md outline-none bg-red-700">Selecciona tus archivos</a>
        </div>
    </div>
    ...
    <form wire:submit.prevent="storePhotos">
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            @if(count($tempFiles) > 0)
                <div class="flex col-span-2 sm:col-span-3 flex-nowrap">
...
{{--                        <input type="checkbox" class="ml-2">--}}
            ...
            @if(count($tempFiles) > 0)
                <input type="submit" value="Subir archivos"
                       class="col-span-2 sm:col-span-3 mt-3 py-1 md:py-2 px-4 text-white text-center font-medium border border-transparent rounded-md outline-none bg-green-700">
            @endif
        </div>
    </form>
